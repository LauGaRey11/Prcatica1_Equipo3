...
static const uint16_t mPanId = 0x0303;
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
    ...
    Serial_Print(interfaceId,"\n\rSource Address: ", gAllowToBlock_d);
    //Serial_PrintHex(interfaceId, pMsgIn->msgData.dataInd.srcAddr, 2, gAllowToBlock_d);
    Serial_PrintDec(interfaceId, pMsgIn->msgData.dataInd.srcAddr);
    ...
    input=*(pMsgIn->msgData.dataInd.pMsdu);
    //Serial_PrintDec(interfaceId, input);
    if (input == 0 ){ //prender led rojo
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnLed(LED2);
    	  }
    	  else if(input == 1){ //prender led verde
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnLed(LED3);
    	  }
    	  else if(input == 2){ //prender led azul
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnLed(LED4);
    	  }
    	  else if(input == 3){ //prender led verde y azul
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnLed(LED3);
    		  LED_TurnOnLed(LED4);
    	  }
    	  else if(input == 4){ //prender led azul y rojo
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnLed(LED4);
    		  LED_TurnOnLed(LED2);
    	  }
    	  else if(input == 5){ //prender todos los led
    		  LED_TurnOffAllLeds();
    		  LED_TurnOnAllLeds();
    	  }
    break;
