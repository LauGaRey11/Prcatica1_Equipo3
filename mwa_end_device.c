...
static tmrTimerID_t EverySecond_Timer = gTmrInvalidTimerID_c;
...
static void App_MyTransmitData(void)
{
    ...
    if(mpPacket != NULL)
    {
        ...
        mpPacket->msgData.dataReq.dstAddrMode = mCoordInfo.coordAddrMode;
        mpPacket->msgData.dataReq.srcAddrMode = mAddrMode;
        mpPacket->msgData.dataReq.msduLength = longitud;
        /* Request MAC level acknowledgement of the data packet */
        mpPacket->msgData.dataReq.txOptions = gMacTxOptionsAck_c;
        /* Give the data packet a handle. The handle is
        returned in the MCPS-Data Confirm message. */
...
static void    MyTimer(void *pData)
{
  ...
  if((segundos & 0x1) == 0){
	  cont++;
	  segundos++;
	  Serial_Print(interfaceId, " Valor del contador:   \n\r", gAllowToBlock_d);
	  Serial_PrintDec(interfaceId, cont);
  }
  else{
	  segundos++;
  }
 ...
 /* Restart timer. */
  TMR_StartSecondTimer(EverySecond_Timer, 1 , MyTimer, NULL );
	  ...
	  if (cont == 0 ){ //prender led rojo
		  LED_TurnOffAllLeds();
		  LED_TurnOnLed(LED2);
	  }
	  else if(cont == 1){ //prender led verde
		  LED_TurnOffAllLeds();
		  LED_TurnOnLed(LED3);
	  }
	  else if(cont == 2){ //prender led azul
		  LED_TurnOffAllLeds();
		  LED_TurnOnLed(LED4);
	  }
	  else if(cont == 3){ //prender led verde y azul
		  LED_TurnOffAllLeds();
		  LED_TurnOnLed(LED3);
		  LED_TurnOnLed(LED4);
	  }
	  else if(cont == 4){ //prender led azul y rojo
		  LED_TurnOffAllLeds();
		  LED_TurnOnLed(LED4);
		  LED_TurnOnLed(LED2);
	  }
	  else if(cont == 5){ //prender todos los led
		  LED_TurnOffAllLeds();
		  LED_TurnOnAllLeds();
	  }
